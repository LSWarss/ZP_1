...
class BookTestCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.book = Book(library=self.library,
                         publication_date=date,
                         author_name="TestName", author_surname="TestSurname",
                         number_of_pages=10)
        self.maxDiff = None

    def test_str(self):
        publicationTest = datetime.datetime.now().strftime("%x")
        self.assertEqual(self.book.__str__(),
                         f"""Book from {self.library.__str__()} publicated on:
 {publicationTest},
